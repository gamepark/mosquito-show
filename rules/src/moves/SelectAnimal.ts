...
export function selectAnimalInView(state: LocalGameView, move: SelectAnimal) {
  state.possibleAnimalFields = []
  var selectedAnimalId = move.selectAnimalId
  state.selectedAnimalId = selectedAnimalId
  var animalFields = state.board.animalLocations
...
export function getCurrentAnimalField(state: GameView) {
  let animalFieldIds = state.board.animalLocations
...
export function isAnimalOnField(state: GameView, fieldId: number) {
  for (let j = 0; j < state.board.animalLocations.length; j++) {
    if (state.board.animalLocations[j].fieldId == fieldId) {
      return true
    }
  }
  return false
